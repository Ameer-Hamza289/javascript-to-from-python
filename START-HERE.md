# Start here

## Pick your path

### A) JS developer → learning Python
Order:
1. Open each lesson’s `js.js` (familiar ground)
2. Open `py.py` (same story in Python)
3. Read `NOTES.md` (differences that matter)
4. Skim [cheatsheet.md](cheatsheet.md) after every 2–3 lessons
5. Do [04-practice](04-practice) exercises in **Python**

### B) Python developer → learning JavaScript
Order:
1. Open each lesson’s `py.py` (familiar ground)
2. Open `js.js` (same story in JS)
3. Read `NOTES.md` (differences that matter)
4. Skim [cheatsheet.md](cheatsheet.md) after every 2–3 lessons
5. Do [04-practice](04-practice) exercises in **JavaScript**

## Suggested pace
- Days 1–3: Fundamentals 01–04
- Days 4–6: Fundamentals 05–08
- Days 7–8: Intermediate
- Days 9–11: OOP
- Day 12+: Practice set + rewrite NOTES you marked

## Study rule
When you see `DIFF` or `GOTCHA`, write one sentence in your own words.
That’s the whole point of this repo — relating what’s the same, owning what isn’t.
